-- Migration 034: Convert vitals_daily from dynamic VIEW to indexed physical TABLE

-- Step 1: Drop old view
DROP VIEW IF EXISTS vitals_daily CASCADE;

-- Step 2: Create physical table directly as vitals_daily
CREATE TABLE IF NOT EXISTS vitals_daily (
    ...
    hr_avg_morning NUMERIC,
    hr_avg_afternoon NUMERIC,
    hr_avg_evening NUMERIC,
    hr_avg_night NUMERIC,
    total_steps NUMERIC,
    distance_meters NUMERIC,
    active_calories NUMERIC,
    sleep_hours NUMERIC,
    oxygen_sat_avg NUMERIC,
    oxygen_sat_min NUMERIC,
    ...
    resting_heart_rate_calculated NUMERIC,
    sleep_max_heart_rate NUMERIC,
    body_temp_avg NUMERIC,
    respiratory_rate_avg NUMERIC,
    avg_speed NUMERIC,
    cough_count_night NUMERIC,
    snoring_events_count NUMERIC,
    sit_to_stand_seconds NUMERIC,
    skin_temperature_delta NUMERIC,
    heart_rate_recovery_calculated JSONB,
    waso_mins NUMERIC,
    awakenings_count BIGINT,
    awakenings_count_greater_than_5mins BIGINT,
    sleep_latency_mins NUMERIC,
    sleep_efficiency_pct NUMERIC,
    sleep_stage_1_pct NUMERIC,
    sleep_stage_2_pct NUMERIC,
    sleep_stage_3_pct NUMERIC,
    sleep_stage_4_pct NUMERIC,
    sleep_stage_5_pct NUMERIC,
    sleep_stage_6_pct NUMERIC,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    CONSTRAINT pk_vitals_daily PRIMARY KEY (patient_id, date)
);

-- Step 3: Indexes
CREATE INDEX IF NOT EXISTS idx_vitals_daily_patient_date 
ON vitals_daily (patient_id, date DESC);

CREATE INDEX IF NOT EXISTS idx_vitals_raw_patient_recorded_at 
ON vitals_raw (patient_id, recorded_at DESC);

-- Step 4: Row Level Security (RLS)
ALTER TABLE vitals_daily ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow select on vitals_daily" ON vitals_daily;
CREATE POLICY "Allow select on vitals_daily" 
ON vitals_daily FOR SELECT 
TO authenticated, anon 
USING (true);

DROP POLICY IF EXISTS "Allow service role all on vitals_daily" ON vitals_daily;
CREATE POLICY "Allow service role all on vitals_daily" 
ON vitals_daily FOR ALL 
TO service_role 
USING (true)
WITH CHECK (true);
...
BEGIN
  ...
  SELECT COALESCE(sk.patient_id, o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id, hrr.patient_id) AS patient_id,
   ...
   FROM (((((((((other_metrics o
    ...
    WHERE (p_patient_id IS NULL OR COALESCE(sk.patient_id, o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id, hrr.patient_id) = p_patient_id)
      AND (p_start_date IS NULL OR COALESCE(sk.date, o.date, h.date, ox.date, s.date, r.date, sp.date, ms.date, spf.date, hrr.date) >= p_start_date)
      AND (p_end_date IS NULL OR COALESCE(sk.date, o.date, h.date, ox.date, s.date, r.date, sp.date, ms.date, spf.date, hrr.date) <= p_end_date)
...
CREATE OR REPLACE FUNCTION trg_fn_sync_vitals_daily()
RETURNS TRIGGER AS $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (
        SELECT DISTINCT 
            patient_id, 
            date_trunc('day', recorded_at AT TIME ZONE COALESCE(timezone, 'UTC')) AS v_date
        FROM new_table
        WHERE is_outlier = false
    ) LOOP
        PERFORM recalculate_vitals_daily(r.patient_id, r.v_date - interval '1 day', r.v_date);
    END LOOP;
    RETURN NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Step 7: Backfill existing data for all patients
DO $$
DECLARE
    p_rec RECORD;
BEGIN
