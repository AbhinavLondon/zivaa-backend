-- Migration 045: Optimize recalculate_vitals_daily (Tier 1 Safe Hotfix)
--
-- Fixes:
-- 1. Unbounded table scan in steps_phone_fallback by adding patient_id and date boundaries.
-- 2. Sleep efficiency percentage clamping to [0.0, 100.0] against sensor rounding anomalies.
-- 3. Deterministic timezone ordering (latest recorded_at per day).
-- 4. Avoid unnecessary trigger cascades in Step 2 with IS DISTINCT FROM.
--
-- Preserves 100% of existing table columns, join topology, and business logic.

-- Step 1: Add partial index on active non-outlier raw vitals
CREATE INDEX IF NOT EXISTS idx_vitals_raw_patient_active 
ON vitals_raw (patient_id, metric_type, recorded_at DESC) 
...
BEGIN
    ...
    WITH ranked_sources AS (
         ...
         SELECT filtered_raw.patient_id,
          ...
          SELECT v.id,
            ...
            dense_rank() OVER (
                PARTITION BY v.patient_id, (date_trunc('day'::text, (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying))))
                ORDER BY COALESCE(
          ...
          FROM (
            SELECT
              v.patient_id,
              date_trunc('day'::text, (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying))) AS date,
              EXTRACT(HOUR FROM (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying))) AS h
            FROM vitals_raw v
            WHERE v.metric_type = 'StepsRecord'::text
              AND v.is_outlier = false
              AND (p_patient_id IS NULL OR v.patient_id = p_patient_id)
              ...
              AND EXTRACT(HOUR FROM (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying))) BETWEEN 8 AND 20
            GROUP BY v.patient_id,
                     date_trunc('day'::text, (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying))),
                     EXTRACT(HOUR FROM (v.recorded_at AT TIME ZONE COALESCE(v.timezone, 'UTC'::character varying)))
  ...
  SELECT 
    COALESCE(mob.patient_id, sk.patient_id, o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id, hrr.patient_id, tz.patient_id) AS patient_id,
    COALESCE(mob.date, sk.date, o.date, h.date, ox.date, s.date, r.date, sp.date, ms.date, spf.date, hrr.date, tz.date) AS date,
    h.avg_heart_rate,
    h.max_heart_rate,
    h.min_heart_rate,
    h.hr_avg_morning,
   ...
   FROM other_metrics o
    ...
    WHERE (p_patient_id IS NULL OR COALESCE(mob.patient_id, sk.patient_id, o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id, hrr.patient_id, tz.patient_id) = p_patient_id)
      AND (p_start_date IS NULL OR COALESCE(mob.date, sk.date, o.date, h.date, ox.date, s.date, r.date, sp.date, ms.date, spf.date, hrr.date, tz.date) >= p_start_date)
      AND (p_end_date IS NULL OR COALESCE(mob.date, sk.date, o.date, h.date, ox.date, s.date, r.date, sp.date, ms.date, spf.date, hrr.date, tz.date) <= p_end_date)
