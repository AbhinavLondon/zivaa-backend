-- Migration 007: Fix total_steps to use phone pedometer as intraday fallback
-- Problem: vitals_daily VIEW only picks the highest-priority source (Samsung Health watch, rank 1).
--          The phone pedometer is unranked (gets 9999) and is always ignored, even when
--          the watch has only synced 10 steps partway through the day.
-- Fix:     1. Add wildcard fallback in ranked_sources for _phone/_watch sources
--          2. Add steps_phone_fallback CTE to capture phone pedometer data
--          3. Use GREATEST(watch_steps, phone_steps) so whichever is higher wins
--          Result: phone fills in during the day, watch total takes over once it's higher.

-- 1. Add the known phone pedometer source to metric_source_priority for explicit matching
INSERT INTO metric_source_priority (metric_type, source, priority_rank)
VALUES 
    ('StepsRecord', 'com.android.healthconnect.phone.j4434491665db35b380e0ce9a1caab17d_phone', 4),
    ('DistanceRecord', 'com.android.healthconnect.phone.j4434491665db35b380e0ce9a1caab17d_phone', 4)
ON CONFLICT DO NOTHING;

-- 2. Recreate the vitals_daily view with steps GREATEST logic and wildcard source fallback
...
-- NEW: Phone pedometer fallback for steps
-- Sums steps per phone source per day, then takes the MAX across phone sources
-- to avoid double-counting if multiple phone apps write step data.
...
sleep_stages AS (
    SELECT filtered_raw.patient_id,
        ...
        (EXTRACT(epoch FROM (((stage.value ->> 'end_time'::text))::timestamp with time zone - ((stage.value ->> 'start_time'::text))::timestamp with time zone)) / 3600.0) AS duration_hours
    FROM filtered_raw,
...
sleep_daily AS (
    SELECT sleep_stages.patient_id,
        sleep_stages.date,
        sum(CASE WHEN (sleep_stages.stage_type = 1) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_1_hours,
        sum(CASE WHEN (sleep_stages.stage_type = 2) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_2_hours,
        sum(CASE WHEN (sleep_stages.stage_type = 3) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_3_hours,
        sum(CASE WHEN (sleep_stages.stage_type = 4) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_4_hours,
        sum(CASE WHEN (sleep_stages.stage_type = 5) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_5_hours,
        sum(CASE WHEN (sleep_stages.stage_type = 6) THEN sleep_stages.duration_hours ELSE (0)::numeric END) AS sleep_stage_6_hours
    FROM sleep_stages
    GROUP BY sleep_stages.patient_id, sleep_stages.date
),
other_metrics AS (
    SELECT filtered_raw.patient_id,
        date_trunc('day'::text, filtered_raw.recorded_at) AS date,
...
SELECT COALESCE(o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id) AS patient_id,
    ...
    h.max_heart_rate,
    h.min_heart_rate,
    -- GREATEST: picks whichever is higher — phone pedometer intraday or watch daily total
