-- Migration 012: Advanced Sleep Metrics (Efficiency, Latency, WASO, Awakenings)
--
-- Developer Notes:
...
SELECT COALESCE(o.patient_id, h.patient_id, ox.patient_id, s.patient_id, r.patient_id, sp.patient_id, ms.patient_id, spf.patient_id) AS patient_id,
    ...
    r.heart_rate_recovery_calculated,
    ...
    COALESCE(adv.waso_mins, 0) AS waso_mins,
    COALESCE(adv.awakenings_count, 0) AS awakenings_count,
    COALESCE(adv.awakenings_count_greater_than_5mins, 0) AS awakenings_count_greater_than_5mins,
