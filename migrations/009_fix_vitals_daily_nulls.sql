-- Migration 009: Fix vitals_daily null aggregation
-- Problem: In the `other_metrics` CTE of `vitals_daily`, `cough_count_night` and `snoring_events_count`
--          used `ELSE (0)::numeric` in the CASE statement. This caused the sum to evaluate to 0
--          instead of NULL whenever other records (like Steps or HeartRate) existed for the day,
--          masking the absence of actual AudioSensorRecords.
-- Fix:     Change `ELSE (0)::numeric` to `ELSE NULL::numeric` for audio metrics. 
