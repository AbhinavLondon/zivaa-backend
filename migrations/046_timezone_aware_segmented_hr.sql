-- Migration 046: Timezone-Aware Segmented Heart Rate & Local Day Alignment
--
-- Fixes:
-- 1. Inherits patients.timezone when vitals_raw.timezone is NULL (~89% of records).
-- 2. Evaluates hr_avg_morning, hr_avg_afternoon, hr_avg_evening, hr_avg_night in patient local time
--    rather than UTC, fixing hour-shifted segment averages across all timezones.
-- 3. Aligns date truncation in granular steps, active hours, and phone fallback to patient local time.
-- 4. Guarantees vitals_daily.timezone is populated with the patient's real IANA timezone.
--
-- Preserves all Tier 1 performance optimizations, table schemas, and join topologies.
